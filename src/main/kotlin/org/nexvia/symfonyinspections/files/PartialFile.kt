...
class PartialFile(viewProvider: FileViewProvider, language: Language) : PsiFileBase(viewProvider, language) {
    ...
    private fun computeIcon(@IconFlags flags: Int): Icon? {
        ...
        if (Registry.`is`("psi.deferIconLoading", true)) {
            ...
            return IconManager.getInstance().withIconBadge(PhpFileType.INSTANCE.icon, JBColor.BLUE)
        }
